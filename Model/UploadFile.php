...
class uploadFile
{
  ...
  public function upload(array $file, string $codeUser, int $codeDetail)
  {
    //Obtener posibles errores
    $error = $this->errores($file);

    if (!$error["error"]) { //Si no hay error

      //Ruta donde se guardan los archivos
      $uploadFileDir = "Files/$codeUser/";
      ...
      if ($result) {
        return ["success" => true, "route" => $dest_path];
      } else {
  ...
  /* Manejar errores */
  private function errores(array $file)
  {
